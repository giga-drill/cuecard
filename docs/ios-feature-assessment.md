...
语音跟读不是简单地把语音转成文字。一个可用版本至少包含实时语音识别、文稿位置匹配、停顿处理、滚动平滑、识别任务恢复和音频中断处理，复杂度中高。第一版现已明确只支持 Apple 系统相机。iOS 是否中断麦克风取决于双方的音频会话类别、混音选项、激活时机和具体设备，并非系统相机必然抢占；已有上架产品证明“PiP 悬浮 + 后台语音跟读 + 外部相机”能够落地，但 Apple 没有公开保证所有机型都能稳定共存。

因此，推荐先做横竖屏，再做一个 1–2 天的 Apple 系统相机真机技术验证。验证通过后进入完整语音跟读实现；若麦克风被中断或端侧识别不可用，产品按已确认规则退回定速滚动，不联网、不扩展内置相机。
...
MIT 开源项目 Open Prompter 采用这类内置相机架构。其成熟语音跟读模块已有数千行生产代码及上千行测试，可作为复杂度参照，而不是直接把语音识别当成几十行 API 接入。它的文稿对齐主要面向英文，本项目需要重写普通话中文的标准化、分词与局部匹配。
...
3. **语音共存验证（1–2 天）**：只接普通话端侧 Speech 流，不做复杂匹配；验证 Apple 系统相机录像时 PiP 后台是否持续收到音频与部分识别结果。
4. **共存结果处理**：验证成功则启用语音跟读；验证失败、被中断或端侧模型不可用时，自动退回定速滚动，不联网、不内置相机。
5. **语音跟读 MVP（7–12 天）**：完成中文文本标准化、局部对齐、滚动协调、中断恢复和单元测试。
6. **真机回归（2–4 天）**：不同机型、方向、长文稿、停顿/口误/跳句及 Apple 系统相机前后摄像头。

## 已确认边界与剩余参数

已确认：第一版只兼容 Apple 系统相机；只做普通话中文；只使用端侧识别；端侧不可用或麦克风被中断时退回定速滚动；不联网、不内置相机；最低 iOS 17.0。

剩余内容不再影响架构，只需在实现时通过默认值和真机测试调优：停顿多久后暂停、重复上一句允许回退多远、连续匹配失败几次后扩大到后两段。建议默认分别从约 1 秒、当前段内回退、连续 3 次失败开始测试。
